...
from misc_multi import (NestedTensor, nested_tensor_from_tensor_list,
...
from deformable_transformer_multi import build_deformable_transformer
import copy
...
def load_trained(model_file):
    if os.path.exists(model_file):
        pt_model = torch.load(model_file)
        model = build_model(pt_model['args'])
        model_layer_dict = model.state_dict()
        pt_layer_dict = pt_model['model']
    else:
        print(f'Error: file: {model_file} does not exist.')
        return None
    
    for layer_name, weight in pt_layer_dict.items():
        if layer_name not in model_layer_dict:
            print(f'{layer_name} not found in model dictionary')
            #return model
        try:
            if weight.shape != model_layer_dict[layer_name].shape:
                print(f'Shape of {layer_name} in pt is {weight.shape}, shape in model is {model_layer_dict[layer_name].shape}')
                #return model
        except:
            None
        model_layer_dict[layer_name] = weight
    
    model.load_state_dict(model_layer_dict, strict=True)
    print(f'Model {model_file} loaded successfully!!')

    return model, pt_model['args']
...
def get_args_parser():
    ...
    parser.add_argument('--lr_drop_epochs', default=[4,6], type=int, nargs='+')
    parser.add_argument('--clip_max_norm', default=0.1, type=float,
                        help='gradient clipping max norm')
    
    parser.add_argument('--num_ref_frames', default=14, type=int, help='number of reference frames')
    ...
    parser.add_argument('--dilation', default=True, action='store_false',
                        help="If false, we do not replace stride with dilation in the last convolutional block (DC5)")
    ...
    # dataset parameters
    parser.add_argument('--dataset_file', default='vid_multi_mm')
...
if __name__ == '__main__':
    ...
    model, args = load_trained('/home/br087771/TransVOD/checkpoint0006.pth')
    model = model.cuda()
    #model = build_model(args).cuda()
    print('Model created Successfully')

    inputs = NestedTensor(torch.randn((1+args.num_ref_frames, 3, 224, 224)).cuda(), torch.randn((1+args.num_ref_frames, 224, 224)).cuda())
    print(f'Inputs: {inputs.tensors.shape}')
    print(f'Num Reference: {args.num_ref_frames}')
